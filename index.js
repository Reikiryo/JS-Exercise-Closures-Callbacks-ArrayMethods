...
function processSum(arr, cb) {
  const sum = arr.reduce((a,c) => {return a + c;}, 0);
  return cb(sum);
}
 ...
 * should return 1000.
*/
function processProduct(n1, n2, cb) {
  return cb(n1 * n2);
}
...
function processDuplicateFree(arr, cb) {
   cb = (el) => el.filter((v,i) => el.indexOf(v) === i);
   return(cb(arr));
...
function getRunnersByTShirtSize(runners, shirt) {
  const newarr = runners.filter(runner => runner.shirt_size === shirt);
  return newarr;
}
...
function tallyUpDonations(runners) {
  const newarr = runners.reduce((acc, cur) => {
